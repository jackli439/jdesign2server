...
(function () {
  ...
  function MainController($http, $log, GraphHelper) {
    ...
    vm.requestSuccessTest;
    vm.requestFinishedTest;
    ...
    function test() {
      ...
      if (expiration > new Date()) {
        ...
        GraphHelper.getFile(0)
          .then(function (response) {
            $log.debug('HTTP request to the Microsoft Graph API returned successfully.', response);
            response.status === 200 ? vm.requestSuccessTest = true : vm.requestSuccessTest = false;
            vm.requestFinishedTest = true;
            console.log(response);
          }, function (error) {
            $log.error('HTTP request to the Microsoft Graph API failed.');
            vm.requestSuccessTest = false;
            vm.requestFinishedTest = true;
          });
       } else {
